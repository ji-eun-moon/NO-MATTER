...
const instance = axios.create({
  baseURL: 'http://172.18.0.3:5000/api/v1', // 원하는 API 서버의 기본 URL을 설정합니다.
});
...
instance.interceptors.response.use(
  (response) => {
    ...
    if (error.response.status === 403 && !originalRequest._retry) {
      ...
      return axios.post('http://172.18.0.3:8080/api/v1/user/refreshToken', { refreshToken: localStorage.getItem('refreshToken') })
        .then((response) => {
