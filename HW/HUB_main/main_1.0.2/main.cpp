...
void publishThread(string topic, string payload) {
    struct mosquitto* mosq_p = mosquitto_new(NULL, true, NULL);
    if (!mosq_p) {
        cout << "Failed to create Mosquitto instance" << endl;
        return;
    }

    int result = mosquitto_connect(mosq_p, "52.79.89.118", 1883, 60); // Mosquitto 브로커 주소와 포트

    if (result != MOSQ_ERR_SUCCESS) {
        cout << "Connection error: " << mosquitto_strerror(result) << std::endl;
    ...
    int publish_mid;
    result = mosquitto_publish(mosq_p, &publish_mid, topic.c_str(), payload.length(), payload.c_str(), 0, false);

    if (result != MOSQ_ERR_SUCCESS) {
        cout << "Publish error: " << mosquitto_strerror(result) << std::endl;
    }
